...
<div id="list">
	<table>
		...
		<tr><td>{$value->user}</td><td>{$value->level}</td><td>{$value->login_count}</td><td>{$value->last_ip}</td><td>{$value->last_time}</td><td><img src="view/admin/images/edit.gif" alt="edit" title="edit"> <a href="?a=manage&m=delete&id={$value->id}" onclick="return confirm('你真的要删除吗？') ? true : false"><img src="view/admin/images/drop.gif" alt="delete" title="delete"></a></td></tr>
		{foreachelse}
